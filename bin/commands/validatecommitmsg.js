...
const MAX_NO_OF_COMMIT = 2;

const validateCommitMsg = () => {
  // Get first and latest commits hash on current branch
  const revHashs = execGitCommand(
    `git rev-list --simplify-by-decoration --no-merges HEAD -${MAX_NO_OF_COMMIT}`
  );

  if (revHashs.length !== MAX_NO_OF_COMMIT) {
